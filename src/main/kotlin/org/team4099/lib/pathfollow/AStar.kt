...
/**
 * Takes two points and runs A* which is a pathfinding algorithm in order to find the most optimal path between two points.
 * Used primarily for on-the-fly trajectory generation.
 */
class AStar {
    ...
    companion object {
        fun solve(startingPosition: Pose2d, endingPosition: Pose2d, grid: List<List<Pose2d>>): List<Pose2d> {
            val yPoints = grid.map { it[0].y }  // Implementation note: Each row of the grid should always have the same y values.

            // Calculate the closest nodes to the starting and ending positions
            val bestYStarting = yPoints.withIndex().minByOrNull { (_, y) -> (startingPosition.y - y).absoluteValue.inMeters }?.index ?: 0
            val bestXStarting = grid[bestYStarting].withIndex().minByOrNull { (_, pose) -> (startingPosition.x - pose.x).absoluteValue.inMeters }?.index ?: 0
            val startingNode = Node(null, grid[bestYStarting][bestXStarting])

            val bestYEnding = yPoints.withIndex().minByOrNull { (_, y) -> (endingPosition.y - y).absoluteValue.inMeters }?.index ?: 0
            val bestXEnding = grid[bestYEnding].withIndex().minByOrNull { (_, pose) -> (endingPosition.x - pose.x).absoluteValue.inMeters }?.index ?: 0
            ...
            while (openList.size >= 1) {  // Keep running until all the nodes are visited. In that case, no optimal path is found.
                currentNode = openList.minByOrNull { it.f } ?: openList.last()  // Find the node with the lowest cost.
                ...
                for ((deltaX, deltaY) in deltas) {
                    ...
                    newNode.h = ((currentNode.position.x - newNode.position.x).absoluteValue.inMeters.pow(2.0) + (currentNode.position.y - newNode.position.y).absoluteValue.inMeters.pow(2.0)).toInt()
                    ...
                    // Discard the node if it is in the open list and has a higher cost than the node already opened.
